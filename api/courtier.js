...
//  Page HTML SSR pour /courtier/:id (agence immobiliere)
//  - select=* (robuste : pas de risque de colonne inconnue)
//  - Deep link lokalist://immo/courtier/:id (l'ecran existe)
// ════════════════════════════════════════════════════════════════
...
<p style="color:#8A8FA8">Ce courtier n'existe plus ou a été retiré.</p>
<p><a href="${SITE_URL}">← Retour à Lokalist</a></p>
</body></html>`;

const pageNotFound = (msg = 'Courtier introuvable') => new Response(html404(msg), {
...
export default async function handler(req) {
  try {
    ...
    const r = await fetch(`${SUPABASE_URL}/rest/v1/courtiers_immo?id=eq.${id}&select=*`, {
    ...
    const description = a.description || `Courtier en crédit${ville ? ' à ' + ville : ''} — sur Lokalist`;
...
<main class="container">
  <article class="hero">
    ...
    <div class="badges"><span class="badge">💼 Courtier en crédit</span></div>
    <div class="head">
