// ════════════════════════════════════════════════════════════════
//  api/mairie.js — Vercel Edge Function
//  Page HTML SSR pour /mairie/:id (événement / actu commune)
//  - Méta Open Graph dynamiques (preview WhatsApp/FB/Twitter)
//  - Capture du code parrain ?ref= -> localStorage (modèle pro.js / i.html)
//  - Deep link lokalist://evenement/:id
//  Table source : evenements_mairie (RLS lecture publique)
// ════════════════════════════════════════════════════════════════
...
const TYPE_LABELS = {
  ...
  conseil:   { label: 'Conseil',   emoji: '🏛️' },
  sport:     { label: 'Sport',     emoji: '⚽' },
  culture:   { label: 'Culture',   emoji: '🎭' },
  autre:     { label: 'Animation', emoji: '📌' },
};
...
// Date FR, fuseau Europe/Paris (fallback si ICU limité sur l'Edge)
...
<p style="color:#8A8FA8">Cet événement n'existe plus ou a été retiré.</p>
...
export default async function handler(req) {
  try {
    ...
    const apiUrl = `${SUPABASE_URL}/rest/v1/evenements_mairie?id=eq.${id}&select=*`;
    ...
    const typeInfo    = TYPE_LABELS[e.type] || TYPE_LABELS.evenement;
...
<main class="container">
  ...
  <section class="section">
    ...
    ${e.lien_inscription ? `<div class="info-row">🔗 <a href="${escapeHtml(e.lien_inscription)}" target="_blank" rel="noopener">Inscription / infos</a></div>` : ''}
  </section>` : ''}

  <div class="cta-block">
    <h3>📅 Suis la vie de ${ville ? escapeHtml(ville) : 'ta commune'} dans l'app</h3>
    <p>Événements, marchés, animations : reste connecté à ta commune et soutiens le local avec Lokalist.</p>
...
<script>
  ...
  // Capture du code parrain (?ref=LOK-XXXXXX) — même stockage que pro.js / i.html
  (function(){
    try {
